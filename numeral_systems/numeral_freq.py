...
def get_google_ngrams(mapping, wordlist, startyear, lang):
    ...
    corpus = {"english": "eng_2012", "chinese": "chi_sim_2012", "french": "fre_2012", "german": "ger_2012",
              "spanish": "spa_2009", "russian": "rus_2012", "hebrew": "heb_2012", "italian": "ita_2012"}
    querylist = [word + "_NUM" + " --startYear=" +
                 str(startyear) + " --endYear=2000" + " --corpus=" + corpus[lang] + " -caseInsensitive" for word in wordlist]
...
def normalize_np(filepath):
    with open(filepath, "r") as f:
        ...
        unnormalized = [float(item)
                        for sublist in unnormalized for item in sublist]
...
def pearson_correlation_num_dig(num_seq, dig_seq, out_file_name, lang, upper_bound=100):
    ...
    plt.plot([i for i in range(upper_bound)], num_seq,
             color="blue", label="Numeral need probability")
    plt.plot([i for i in range(upper_bound)], dig_seq,
             color="orange", label="Digit need probability")
    plt.legend()
    plt.xlabel("Number")
    plt.ylabel("Need probability")
    plt.title("Correlation between " + lang +
              " numerals and digit need probability")
...
def count_ngrams(phrase, length, lang):
    ...
    count = 0
    chinese_character_to_sound = {u'\u5341': 's', u'\u4e8c': 'e', u'\u4e09': 's', u'\u56db': 's',
                                  u'\u4e94': 'w', u'\u516d': 'l', u'\u4e03': 'q', u'\u516b': 'b', u'\u5e5d': 'j'}
    ngram_downloader_langcode = {"english": "eng", "chinese": "chi-sim", "french": "fre",
                                 "german": "ger", "hebrew": "heb", "italian": "ita", "russian": "rus", "spanish": "spa"}
    ...
    fname, url, records = next(readline_google_store(
        ngram_len=length, lang=ngram_downloader_langcode[lang], indices=all_combinations))
...
def collect_lang_data_words(filepath, word_to_num_map, dict_name="total_word_freq_num.p", decades="all", langs="all"):
    freq = {}
    POS = ["_NOUN", "_NUM", "_VERB", "_PRT", "_ADV",
           "_ADJ", "_PRON", "_DET", "_ADP", "_CONJ"]
    chinese_character_to_sound = {u'\u5341': 's', u'\u4e8c': 'e', u'\u4e09': 's', u'\u56db': 's',
                                  u'\u4e94': 'w', u'\u516d': 'l', u'\u4e03': 'q', u'\u516b': 'b', u'\u5e5d': 'j'}
    for root, dirs, files in os.walk(filepath):
        for d in dirs:
    ...
    for lang in freq:  # iterate through unigram files (stored locally)
        if lang == "chinese":
            first_letters = set([chinese_character_to_sound[key]
                                 for key in chinese_character_to_sound])
        else:
            first_letters = set([str(unicode(key)[0])
                                 for key in word_to_num_map[lang].keys()])
        ...
        unigrams = [phrase for phrase in word_to_num_map[lang]
                    if len(phrase) == 1]
        bigrams = [phrase for phrase in word_to_num_map[lang]
                   if len(phrase) == 2]
        trigrams = [phrase for phrase in word_to_num_map[lang]
                    if len(phrase) == 3]
        fourgrams = [phrase for phrase in word_to_num_map[lang]
                     if len(phrase) == 4]
        fivegrams = [phrase for phrase in word_to_num_map[lang]
                     if len(phrase) == 5]
    ...
    for s_r, s_d, s_f in os.walk(filepath):
        for mapping_file in s_f:
            if mapping_file[:-4] in langs or langs == "all":
                ...
                f = codecs.open(filepath + "/" + mapping_file,
                                "r", encoding="utf-8")
                lines = f.read().split('\r\n')[:-1]
                df = pd.read_csv(filepath + "/" +
                                 mapping_file, encoding="utf-8")
                mapping[mapping_file[:-4]
                        ] = pd.Series(df.Number.values, index=df.Reading).to_dict()
...
if __name__ == "__main__":
    ...
    eng_digits = [float(item) for sublist in list(eng_reader)
                  for item in sublist]
    chn_digits = [float(item) for sublist in list(chn_reader)
                  for item in sublist]
    fre_digits = [float(item) for sublist in list(fre_reader)
                  for item in sublist]
    ger_digits = [float(item) for sublist in list(ger_reader)
                  for item in sublist]
    ita_digits = [float(item) for sublist in list(ita_reader)
                  for item in sublist]
    heb_digits = [float(item) for sublist in list(heb_reader)
                  for item in sublist]
    rus_digits = [float(item) for sublist in list(rus_reader)
                  for item in sublist]
    spa_digits = [float(item) for sublist in list(spa_reader)
                  for item in sublist]
    ...
    sorted_mapping = OrderedDict(
        sorted(mapping["italian"].iteritems(), key=lambda (k, v): (v, k)))
    print(sorted_mapping)
    french_words = [key for key in sorted_mapping.keys()]
    total_dict = get_google_ngrams(
        sorted_mapping, french_words, 1900, "italian")
    print(total_dict)

    plot_languages(ld, mode=2, filename="need_probs.png",
