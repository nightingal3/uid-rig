...
def err_over_time(filename):
    """Old idea: for each number word, break it into components and let the error be the distance from the target.
    Example: "twenty-three" has a cumulative error of 3, "three-twenty" has a cumulative error of 17"""
    ...
    chn_dictionary = {"yi": 1, "er": 2, "san": 3, "si": 4, "wu": 5, "liu": 6, "qi": 7, "ba": 8, "jiu": 9, "shi": 10,
                      "ershi": 20, "sanshi": 30, "sishi": 40, "wushi": 50, "liushi": 60, "qishi": 70, "bashi": 80, "jiushi": 90}
    reverse_dictionary_eng = {value: key for key,
                              value in eng_dictionary.items()}
    reverse_dictionary_chn = {value: key for key,
                              value in chn_dictionary.items()}
    f = open(filename, "r")
    eng_points = []
    eng_mod_points = []
    chn_points = []
    ...
    for i in range(len(number)):
        ...
        for j in range(len(eng_words[i])):
            if j == 0:
                temp.append(
                    (len(eng_words[i][j]), number[i], eng_dictionary[eng_words[i][j]], j))
            else:
                temp.append(
                    (len(eng_words[i][j]), number[i], eng_dictionary[eng_words[i][j]] + eng_dictionary[eng_words[i][j - 1]], j))
        seq_eng_0.extend(temp)
        temp1 = []
        for j in range(len(eng_mod[i])):
            if j == 0:
                temp1.append(
                    (len(eng_mod[i][j]), number[i], eng_dictionary[eng_mod[i][j]], j))
            else:
                temp1.append(
                    (len(eng_mod[i][j]), number[i], eng_dictionary[eng_mod[i][j]] + eng_dictionary[eng_mod[i][j - 1]], j))
    ...
    for i in range(len(seq_eng_0)):
        if seq_eng_0[i][3] == 0:
            res0.append((seq_eng_0[i][0], find_err(
                seq_eng_0[i][1], seq_eng_0[i][2]), seq_eng_0[i][1]))
        else:
            res0.append((seq_eng_0[i][0] + seq_eng_0[i - 1][0],
                         find_err(seq_eng_0[i][1], seq_eng_0[i][2]), seq_eng_0[i][1]))

    for i in range(len(seq_eng_1)):
        if seq_eng_1[i][3] == 0:
            res1.append((seq_eng_1[i][0], find_err(
                seq_eng_1[i][1], seq_eng_1[i][2]), seq_eng_1[i][1]))
        else:
            res1.append((seq_eng_1[i][0] + seq_eng_1[i - 1][0],
                         find_err(seq_eng_1[i][1], seq_eng_1[i][2]), seq_eng_1[i][1]))
...
def err_over_time_2(filename, col_num, langdict, charcount=False):
    ...
    for i in range(len(number)):
        ...
        for j in range(len(words[i])):
            if j == 0:
                if charcount:
                    temp.append(
                        (len(words[i][j]), number[i], langdict[words[i][j]], j))
                else:
                    temp.append((j + 1, number[i], langdict[words[i][j]], j))
            else:
                if charcount:
                    temp.append(
                        (len(words[i][j]), number[i], langdict[words[i][j]] + langdict[words[i][j - 1]], j))
                else:
                    temp.append(
                        (j, number[i], langdict[words[i][j]] + langdict[words[i][j - 1]], j))
        seq.extend(temp)
        temp1 = []
        for j in range(len(mod_words[i])):
            if j == 0:
                if charcount:
                    temp1.append(
                        (len(mod_words[i][j]), number[i], langdict[mod_words[i][j]], j))
                else:
                    temp1.append(
                        (j + 1, number[i], langdict[mod_words[i][j]], j))
            else:
                if charcount:
                    temp1.append(
                        (len(mod_words[i][j]), number[i], langdict[mod_words[i][j]], j))
                    ...
                    temp1.append(
                        (j, number[i], langdict[mod_words[i][j]] + langdict[mod_words[i][j - 1]], j))
    ...
    for i in range(len(seq)):
        if seq[i][3] == 0:
            ...
            res0.append((seq[i][0] + seq[i - 1][0],
                         find_err(seq[i][1], seq[i][2]), seq[i][1]))

    for i in range(len(seq_mod)):
        if seq_mod[i][3] == 0:
            res1.append((seq_mod[i][0], find_err(
                seq_mod[i][1], seq_mod[i][2]), seq_mod[i][1]))
        else:
            res1.append((seq_mod[i][0] + seq_mod[i - 1][0],
                         find_err(seq_mod[i][1], seq_mod[i][2]), seq_mod[i][1]))
...
def find_area(opt1, opt2):
    ...
    opt1_area = sum([opt1_width[i] * opt1_height[i]
                     for i in range(len(opt1_width))])
    ...
    opt2_area = sum([opt2_width[i] * opt2_height[i]
                     for i in range(len(opt2_width))])
